...
分隊は「ゼロから版面を自由設計する」のが苦手（実測：座標も配色も丸投げすると
毎回破綻する）。逆に「既存の COMPOSE 設計書（design_*.json）を渡し、
一部分だけ変える」指示は成功しやすい。

- 2案目以降を出す時は、**既にある design_*.json を型として渡し、
  差分（背景色・コピー・配置のどこを変えるか）だけを指示する**
- 1指示＝1周回＝1ファイルの粒度を守る（版面を丸ごと作り直す指示は分割する）
- 美的な良し悪しの判断は分隊にさせない。人間が検収する前提で、
  分隊は「指示された差分を機械的に反映する」役に徹させる
